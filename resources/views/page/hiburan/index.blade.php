            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden lg:col-span-1">
                    <div class="p-6 border-b border-gray-50">
                        <h3 class="text-lg font-medium text-gray-800 tracking-wide">Data Hiburan Baru</h3>
                        <p class="text-xs text-gray-400 font-light mt-1">Tambahkan paket *entertainment* / hiburan acara.</p>
                    </div>
                    <div class="p-6">
                        <form method="POST" action="{{ route('hiburan.store') }}" class="space-y-4">
                            @csrf
                            <div>
                                <x-input-label for="type_hiburan" value="Jenis Hiburan" class="text-gray-600 mb-1.5 text-xs" />
                                <x-text-input type="text" name="type_hiburan" class="block w-full border-gray-200 rounded-xl text-sm focus:border-zinc-900 focus:ring-zinc-900/20" required placeholder="Contoh: Akustik Band" />
                            </div>
                            <div>
                                <x-input-label for="deskripsi" value="Deskripsi & Detail" class="text-gray-600 mb-1.5 text-xs" />
                                <textarea name="deskripsi" rows="3" class="block w-full border-gray-200 rounded-xl text-sm focus:border-zinc-900 focus:ring focus:ring-zinc-900/20 text-gray-700 transition-colors resize-none" required placeholder="Jelaskan formasi pemain, alat musik, atau durasi main..."></textarea>
                            </div>
                            <div>
                                <x-input-label for="harga" value="Harga Sewa (Rp)" class="text-gray-600 mb-1.5 text-xs" />
                                <x-text-input type="number" name="harga" class="block w-full border-gray-200 rounded-xl text-sm focus:border-zinc-900 focus:ring-zinc-900/20" required placeholder="2500000" />
                            </div>
                            <div class="pt-3">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden lg:col-span-2">
                    <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-medium text-gray-800 tracking-wide">Katalog Hiburan</h3>
                            <p class="text-xs text-gray-400 font-light mt-1">Daftar layanan hiburan yang tersedia untuk acara.</p>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-600">
                            <thead class="text-xs text-gray-400 uppercase tracking-wider bg-slate-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4 font-medium">Jenis Hiburan</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse ($hiburans as $index => $hiburan)
                                    <tr class="bg-white hover:bg-slate-50/50 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-gray-500 text-xs font-light mb-1 line-clamp-2 max-w-xs">{{ $hiburan->deskripsi }}</div>
                                            <div class="font-medium text-emerald-600">Rp {{ number_format($hiburan->harga, 0, ',', '.') }}</div>
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                <p class="text-gray-400 font-light">Belum ada paket hiburan yang terdaftar.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl mx-4 transform transition-all">
            ...
            <form method="POST" id="formSourceModal">
                ...
                <div class="px-6 py-5 space-y-4">
                    <div>
                        <x-input-label for="modal_type_hiburan" value="Jenis Hiburan" class="text-gray-600 mb-1.5 text-xs" />
                        <x-text-input type="text" id="modal_type_hiburan" name="type_hiburan" class="block w-full border-gray-200 rounded-xl text-sm focus:border-zinc-900 focus:ring-zinc-900/20" />
                    </div>
                    <div>
                    ...
                    <div>
                        <x-input-label for="modal_harga" value="Harga Sewa (Rp)" class="text-gray-600 mb-1.5 text-xs" />
...
<script>
    ...
    const editSourceModal = (button) => {
        ...
        document.getElementById('title_source').innerText = `Edit: ${type_hiburan}`;
    ...
    const hiburanDelete = (id, type_hiburan) => {
        Swal.fire({
            title: 'Hapus Data?',
            text: `Paket Hiburan "${type_hiburan}" akan dihapus permanen!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                .then(function(response) {
                    Swal.fire({
                        title: 'Terhapus!',
                        text: 'Data hiburan berhasil dihapus.',
                        icon: 'success',
